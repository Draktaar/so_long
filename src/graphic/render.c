...
/*   Updated: 2025/04/10 14:53:45 by achu             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	for (size_t i = 0; i < 63; i++)
	{
		...
		for (int y = 0; y < game->solids[i].collider.size.y; y++) {
			for (int x = 0; x < game->solids[i].collider.size.y; x++) {
				...
				ft_pixel_put(&game->buffer, game->solids[i].collider.pos.x + x,  game->solids[i].collider.pos.y + y, color);
			}
		}
	}
	for (size_t i = 0; i < 63; i++)
		draw_rect(&game->buffer, game->solids[i].collider, RED);
	draw_rect(&game->buffer, game->player.col, WHITE);
	pixel_scale(&game->buffer, &game->screen, 4);
